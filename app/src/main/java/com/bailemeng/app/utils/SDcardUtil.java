...
public final class SDcardUtil {
	...
	/** 默认根目录名称 */
	private static String ROOT_DIR     = "artist";
	/** 默认图片目录名称 */
	...
	/** 默认epub的目录 */
	private static File epubDirFile  = null;
	...
	public static void initDir() {
		...
		//默认epub目录
		String epubDownloadPath = cacheDownloadPath + EPUB_DIR + File.separator;
		...
		try {
			...
			epubDirFile = makeDir(root.getAbsolutePath() + epubDownloadPath);
		}
		catch (Exception e){
			e.printStackTrace();
		}
	 ...
	 * @return File
	 */
	public static String makeBooksDir(String path) {
		File file = new File(bookDirFile.getPath() + path);
		if (!file.exists()){
			file.mkdirs();
		}
		return file.getPath();
	 ...
	 * @return
     */
	public static File getMarkBookDirFile(String lastName){
		File file = new File(getBookDir() + "/" + lastName);
		if (!file.exists()){
			file.mkdirs();
		}
		return file;
